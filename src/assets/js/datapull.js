...
function openbtmmenu() {
 ...
 myDiagram.contextMenu = myContextMenu;
  ...
  //above
    ...
    var settings = {
      url:
        "http://apiserver.centralindia.azurecontainer.io:5000///dataset/get/",
      method: "GET",
      timeout: 0,
    };
    ...
    $.ajax(settings).done(function (response) {
      var text = "";
      var y = JSON.parse(response).details.details;
      var x = JSON.parse(y);
      for (i in x) {
        text +=
          '<li class="nav-item px-3"><a class="nav-link text-dark f-13">' +
          x[i].datasourcename +
          "</a></li>";
      }
